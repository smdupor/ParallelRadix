#!/bin/bash
#SBATCH --job-name=frSerRad     # Job name
#SBATCH --mail-type=END,FAIL         # Mail events (NONE, BEGIN, END, FAIL, ALL)
#SBATCH --mail-user=smdupor@ncsu.edu    # Where to send mail	
#SBATCH -p skylake
#SBATCH --nodes=1                    # Run all processes on a single node
#SBATCH --time=10:15:00              # Time limit hrs:min:sec
#SBATCH --output=frSerRad-%j.log     # Standard output and error log
pwd; hostname; date

echo "Running Serial Radix Sort for All dataset on $SLURM_CPUS_ON_NODE CPU cores"
